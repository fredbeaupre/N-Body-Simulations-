...
class Quad:
    ...
    def addBody(self, idx, depth):
        ...
        if len(self.bods) > 0 or not self.leaf:
            ...
            self.com = (self.com * self.mass +
                        POS[idx] * bodyMass) / (self.mass + bodyMass)
            self.mass += bodyMass
            # self.updateCOM()
        else:
    ...
    def updateCOM(self):
        if self.leaf:
            ...
            self.com = array(
                map(lambda x: POS[x]*MASS[x], self.bods)).sum(0) / self.mass
        else:
            self.mass = array(
                map(lambda child: child.mass if child else 0, self.children)).sum()
            # print array(map(lambda child: child.mass*child.com if child else zeros(2), self.children)).sum(0) / self.mass
            # raise "PAUSE"
            self.com = array(map(lambda child: child.mass *
                             child.com if child else zeros(2), self.children)).sum(0) / self.mass
    ...
    def __str__(self):
        ...
        if any(self.children):
            childCount = "C:%g," % sum(
                map(lambda x: 1 if x else 0, self.children))
            childStr = "\n"
            for x in range(4):
...
class BoundingBox:
    def __init__(self, box, dim=2):
        ...
        self.center = array([(self.box[2]+self.box[0])/2,
                            (self.box[3]+self.box[1])/2], dtype=float)
        self.dim = dim
...
def drawAccelGrid(drawPtr, gridSize):
    ...
    for x in range(gridSize):
        for y in range(gridSize):
            p = array([x+0.5, y+0.5], dtype=float) * \
                BOUNDS.sideLength / gridSize
...
def drawBodies(drawPtr):
    for x in range(N):
        if BOUNDS.inside(POS[x]):
            ...
            print(type(p))
            print(type(r))
            drawPtr.ellipse(join(p-r, p+r), fill=(0, 0, 255))
...
def drawBBOX(drawPtr, node):
    ...
    # print "Draw box %s " % p
    drawPtr.rectangle(p, outline=(0, 0, 0))
...
def drawCross(drawPtr, p):
    p = convertPos(p)
    # print "Cross %s" % p
    drawPtr.line((p[0]-CROSS_SIZE, p[1], p[0] +
                 CROSS_SIZE, p[1]), fill=(0, 255, 0))
    drawPtr.line((p[0], p[1]-CROSS_SIZE, p[0],
                 p[1]+CROSS_SIZE), fill=(0, 255, 0))
...
def convertPos(p):
    ...
    # p = (x,y) -> [ix, iy]
    c = trunc((p - BOUNDS.min()) / (BOUNDS.max()-BOUNDS.min())
              * array(IMAGE_SIZE)).tolist()
